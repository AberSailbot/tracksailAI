...
public class Game implements Runnable, Serializable{
	...
	/**
	 * list of results is a <code>String</code> array where the name of each
	 * player reaching the goal is stored
	 */
	...
	/**
	 * Is the race being run? The race loop is executed as long as running ==
	 * true.
	 */
	...
	public GameInfo getInfo(){
		return new GameInfo(getId(), getName(), getMasterId(), getNumPlayers(),
				isRacing());
	}
	...
	public synchronized void loadTrack(String track_name){
		...
		// Load a new track
		try{
			new_track = (new TrackFileParser()).parseFromWaypointFile(track_name);
	 ...
	 * 
	 * @param p
	 *            <code>Player</code> object to add
	 ...
	 * 
	 * @param min
	 *            lower limit of the integer to return
	 * @param max
	 *            the upper limit of the integer to return
	 * @return A random integer in the given range.
	 */
	public int getRandomIntRange(int min, int max){
		if(min == max)
			return min; // If they are the same, we would divide by zero
		int r = rand.nextInt();
		if(r < 0)
			r = -r;
	...
	public Game(int id, String name, int master_id, Server s){
		...
		loadTrack("../tracks/waypoints.txt");
	...
	public void startGame(){
		...
		synchronized(this){
			...
			while(it.hasNext()){
				...
				try{
					p.getClient().gameStateChange(is_racing);
	...
	public void run(){
		...
		while(running){
			...
			if(current_time > change_wind_time){
				wind_direction = getRandomIntRange(track.getMinWindDirection(),
						track.getMaxWindDirection());
				wind_velocity = getRandomIntRange(track.getMinWindVelocity(),
						track.getMaxWindVelocity());
			...
			if(is_racing){ // yes
				...
				if(num_finished > 0){
					setStatusText("Race won by " + race_result[0]
							+ "! Time left: " + (end_time - current_time));
					System.out.println("Race won by " + race_result[0]
							+ "! Time left: " + (end_time - current_time));
	 ...
	 * 
	 * @param sedconds
	 *            How long a time slice will we move, given in seconds.
	 */
	public synchronized void movePlayers(float seconds){
		...
		while(it.hasNext()){
			...
			// if( !p.isFinished() && track.passedPort( p.getNextPort(),
			// old_pos, new_pos ) )
	...
	public void gotoNextWp(Player p){
		...
		if(p.getCurLap() > track.getNumberOfLaps()){
			...
			race_result[num_finished] = p.getName() + " " + (s / 60) + ":"
					+ (s % 60);
			num_finished++;
	 ...
	 * 
	 * @param plr
	 *            The player whose collisions to check.
	 */
	 ...
	 * <p>
	 * NOTE: We only write the necessary attributes and objects to keep the race
	 * and network load as load as possible.
	 * 
	 * @param out
	 *            <code>ObjectOutputStream</code> to write into
	 */
	private synchronized void writeObject(java.io.ObjectOutputStream out)
			throws IOException{
		out.writeObject(players); // ArrayList is Serializable, this should be
									// ok.
